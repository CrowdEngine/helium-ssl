...
group=$(echo $tags | jq -r '.Tags[] | select(.Key == "Group") | .Value')
env=$(echo $tags | jq -r '.Tags[] | select(.Key == "Environment") | .Value')
name="$group-$env $instance_id"
aws ec2 create-tags --resources $instance_id --tags "Key=Name,Value=$name" --region $instance_region
